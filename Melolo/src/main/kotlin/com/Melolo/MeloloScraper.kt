        ...
        return runCatching {
            ...
            coroutineScope {
                allPossibleLinks.filter { it.first.isNotBlank() }.map { (raw, label) -> async { runCatching {
                    ...
                    val okDirect = runCatching { loadExtractorWithFallbackCustom(fixedUrl, currentUrl, subtitleCallback, callback) }.getOrElse { 
                        safeReportError(provider.providerId, it)
                        false 
                    }
                    if (!okDirect) {
                        ...
                        val okRecursive = runCatching { loadExtractorWithFallbackCustom(finalIframe, refererForExtractor, subtitleCallback, callback) }.getOrElse {
                            safeReportError(provider.providerId, it)
                            false
                        }
                    ...
                    logDebug(provider.providerId, "Link Processor Error: ${e.message}")
                    safeReportError(provider.providerId, e)
                } } }.awaitAll()
            }
            true
