...
object LicenseClient {
    ...
    private suspend fun discoverKey(): String? {
        ...
        } catch (e: Exception) { Log.w(TAG, "Key discovery failed: ${e.message}"); null }
    }
    ...
    suspend fun checkLicense(pluginName: String, action: String = "OPEN", data: String? = null): Boolean {
        ...
        val throttleMs = when (action.uppercase()) {
            "HOME" -> 60_000L; "SEARCH" -> 10_000L; else -> 5_000L
        }
        ...
        if (cachedStatus == "active" && now < cacheExpiry && action.uppercase() != "PLAY") {
            logActionAsync(pluginName, action, data); return true
        }
        ...
        return try {
            ...
            val cleanPlugin = pluginName.replace("\"", ""); val cleanAction = action.replace("\"", ""); val cleanData = (data ?: "").replace("\"", "")
            ...
            if (json?.status == "active" || json?.status == "success") {
                ...
                if (reason == "not_found" || reason == "revoked") appContext?.getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE)?.edit()?.remove(PREF_KEY)?.apply()
                false
            }
        } catch (e: Exception) {
    ...
    private suspend fun getPluginSessionToken(pluginName: String): String? {
        ...
        return try {
            ...
            if (json?.status == "ok" && !json.sessionToken.isNullOrEmpty()) {
                ...
                licenseBlocked = false; blockMessage = ""; json.sessionToken
            } else {
                pluginSessionToken = null; pluginSessionPlugin = null; pluginSessionExpiry = 0L
                licenseBlocked = true; blockMessage = json?.message ?: "Session plugin tidak valid"; null
            }
        } catch (e: Exception) {
    ...
    suspend fun getSelectors(pluginName: String): SelectorConfig? {
        ...
        return try {
            ...
            val body = jsonPayload.toRequestBody("application/json".toMediaTypeOrNull())
            val response = app.post("$SERVER_URL/api/selectors", headers = mapOf("Authorization" to "Bearer $sessionToken"), requestBody = body).text
            ...
            if (json?.status == "ok" && json.selectors != null) {
                ...
                licenseBlocked = true; blockMessage = json?.message ?: "Selector tidak tersedia"; selectorCache.remove(pluginName); null
            }
        } catch (e: Exception) {
    ...
    data class SelectorConfig(val playerSelector: String = ".mobius option", val playerAttr: String = "value", val useBase64: Boolean = true)
